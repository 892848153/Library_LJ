...
public class ImageGetFromHttp {
    ...
    /**
     * @param context
     * @param memoryCach         下载完图片是否缓存在内存中
     * @param diskCach           下载完图片是否缓存在sdcard上
     * @param recycleMemoryCache 下载完图片是否缓存在可以回收的内存中，该片缓存可更加cacheFlag回收
     */
     ...
     *
     * @param url
     * @param cacheFlag recycleMemoryCache为true时，可回收内存区域中，可根据cacheFlag回收相应的内存
     * @param imageView 不想显示可以传入null
     * @return
     ...
     * @param url
     * @param imageView
     * @param targetWidth     图片将要压缩的目标宽度
     * @param targetHeight    图片将要压缩的目标高度
     * @param recycleOnFinish 是否将其加入内存缓存中的特殊队列中，为true时，调用
    ...
    private void sendRequest(String url) {
        ...
        if (Build.VERSION.SDK_INT < 11) {
            ...
            NetworkChecker.NetworkType type = NetworkChecker.getNetworkType(mContext);
            switch (type) {
