...
/**
 * 弹性滚动.
 * Created by liujie_gyh on 16/3/18.
 */
public class BoundScrollView extends ScrollView {
    ...
    public boolean dispatchTouchEvent(MotionEvent ev) {
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                //是否应该移动布局
                boolean shouldMove =
                        (mCanPullDown && deltaY > 0)    //可以下拉， 并且手指向下移动
    ...
    protected boolean isCanPullDown() {
        //因为这种类型的控件的内部会滚动, 所以getScrollY()永远返回0，不能用getScroll()方法判断是否滚动了
        if (isAbsListView()) {
    ...
    /**
     * 是否是ListView, GridView这种类型的控件。
     *
     * @return
     */
    private boolean isAbsListView() {
        return mContentView instanceof AbsListView ? true : false;
    }
}
